...
function Signup({ URL }) {
    ...
    function handleCredentialChange(e) {
        ...
        if (clickSendOTP && e.target.name !== 'otp') {
            ...
            setErrors(tempError)
            // console.log(credentials,tempError,e.target.name,e.target.value)
        }
        else {
            setErrors({})
        }
    ...
    async function otpsubmit(e) {
        ...
        if (!tempError.otp) {
            ...
            if (credentials.babyDOB !== '') {
                form.append('babyDOB', credentials.babyDOB)
            }
            if (credentials.bname !== '') {
                form.append('bname', credentials.bname)
            }
            const fetchReq = {
                method: 'POST',
                body: form
            }
            const response = await fetch(URL + 'auth/signup/', fetchReq)
    ...
    async function generateOTP(e) {
        ...
        else if (tempError.babyDOB) {
            babyDOBInputRef.current.focus();
        }
                                    ...
                                    <div className="container flex items-center justify-center min-h-screen px-6 mx-auto">
                                        <div className="w-full max-w-md">
                                            <form className="w-full max-w-md" onSubmit={generateOTP}>
                                                ...
                                                <div className='mt-4'>
                                                    <label htmlFor="Birthday" className="block text-sm font-bold text-gray-500 dark:text-gray-300">Baby DOB</label>
                                                    <input
                                                        ref={babyDOBInputRef}
                                                        type="date"
                                                        name='babyDOB'
                                                        value={credentials.babyDOB}
                                                        onChange={handleCredentialChange}
                                                        placeholder="Baby DOB"
                                                        className={`block w-full py-3 text-gray-700 bg-white border rounded-lg px-11 dark:bg-gray-900 dark:text-gray-300 ${errors.babyDOB == null ? 'dark:border-gray-600 focus:border-blue-400 dark:focus:border-blue-300 focus:ring-blue-300' : 'border-red-600 focus:border-red-400 dark:focus:border-red-300 focus:ring-red-300'} focus:outline-none focus:ring focus:ring-opacity-40`}
